...
class Audio:
    ...
    def read_mic(self,seconds=5):
        ...
        CHANNELS = 1
        RATE = 44100 #For some reason the stream reads at twice the input rate. -_- ?
        p = pyaudio.PyAudio()
    ...
    def play_audio(self,soundArray):
        ...
        stream = p.open(format=FORMAT,
                        ...
                        output=True,
                        frames_per_buffer=CHUNK)
        stream.write(soundArray, num_frames=None, exception_on_underflow=False)#repeat the data because pyaudio plays only half of the data -_- ?
        stream.stop_stream()
        stream.close()
        p.terminate()